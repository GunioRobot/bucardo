...
($res, @dummy) = split "\n", $bct->ctl('add table makedelta      db=A herd=herdA target_makedelta=on');
...
$res = $bct->ctl('add sync makedeltasync_a source=herdA type=pushdelta targetdb=B target_makedelta=on');
chomp $res;
like($res, qr/Added sync/, $res);
$dbhX->commit();
